...
namespace NzbDrone.Core.Validation
{
    public static class RuleBuilderExtensions
    {
        ...
        {
            return ruleBuilder.Matches(@"^(?!\/?https?://[-_a-z0-9.]+)", RegexOptions.IgnoreCase)
                              .WithMessage($"Must be a valid URL path (ie: '{example}')");
        }
        ...
        {
            ruleBuilder.NotEmpty();
            return ruleBuilder.Matches("whisparr", RegexOptions.IgnoreCase).WithMessage("Must contain Whisparr");
        }
